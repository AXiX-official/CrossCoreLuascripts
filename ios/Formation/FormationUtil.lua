...
--读取2D的头像
function this.Load2DImg(go,iconName,gridType)
	if go and iconName then
		local type=FormationUtil.GetFormationType(gridType);
		if type == nil or type == FormationType.Single then
			ResUtil.FormationIcon:Load(go,iconName.."_N");
		elseif type == FormationType.VDouble or type==FormationType.VThree or type==FormationType.Summon or type==FormationType.VDThree then 
			ResUtil.FormationIconV:Load(go,iconName.."_V");
		elseif type == FormationType.HDouble or type == FormationType.HThree  then
			ResUtil.FormationIconH:Load(go,iconName.."_H");
		elseif type == FormationType.Square or type==FormationType.Nine then
			ResUtil.FormationIcon:Load(go,iconName.."_N");
		end
	end
end
