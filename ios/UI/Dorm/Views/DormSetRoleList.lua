...
function Awake()
    ...
    layout:Init("UIs/CRoleItem/CRoleItem", LayoutCallBack, true)
...
function OnInit()
    UIUtil:AddTop2("DormSetRoleList", gameObject, function()
        view:Close()
    end, nil, {})
end
...
function ItemClickCB(item)
    -- 重复的不能选
    if (maxCount > 1 and item.isRePe and not item.isSelect) then
        return
    end
...
function InitDataBase()
    ...
    if (roomType == RoomType.building) then
        ...
        oldSelectIDs = roomData:GetRoles()
        maxCount = roomData:GetMaxNum()
    end
...
function ShowList()
    ...
    curDatas = SortMgr:Sort(sortId, baseDatas, roomId)
...
function SetCurSelectPanel()
    curData = nil
    if (curID) then
        curData = CRoleMgr:GetData(curID)
    end
...
function OnClickSure()
    ...
    if (isChange) then
        ...
        for i, v in pairs(selectIDS) do
            local _data = CRoleMgr:GetData(i)
...
function IsRepe(_data)
    ...
    if (repleaceType ~= nil or sameIsReplace ~= nil) then
        for k, v in pairs(selectIDS) do
            local cRoleInfo = CRoleMgr:GetData(k)
