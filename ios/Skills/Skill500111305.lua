...
---------------------------------------------
-- 技能基类
Skill500111305 = oo.class(SkillBase)
function Skill500111305:Init(skillID, card)
	SkillBase.Init(self, skillID, card)
end
-- 执行技能
function Skill500111305:DoSkill(caster, target, data)
	-- 11004
	self.order = self.order + 1
	self:DamagePhysics(SkillEffect[11004], caster, target, data, 0.25,4)
	-- 500100321
	self.order = self.order + 1
	self:HitAddBuff(SkillEffect[500100321], caster, target, data, 10000,500100321)
	-- 500100322
	self.order = self.order + 1
	self:HitAddBuff(SkillEffect[500100322], caster, target, data, 10000,500100322)
end
-- 行动结束
function Skill500111305:OnActionOver(caster, target, data)
	-- 8060
