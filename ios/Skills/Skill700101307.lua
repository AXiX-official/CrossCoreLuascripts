...
---------------------------------------------
-- 技能基类
Skill700101307 = oo.class(SkillBase)
function Skill700101307:Init(skillID, card)
	SkillBase.Init(self, skillID, card)
end
-- 执行技能
function Skill700101307:DoSkill(caster, target, data)
	self.order = self.order + 1
	self:DamageSpecial(SkillEffect[13003], caster, target, data, 0.333,3)
end
-- 行动结束
function Skill700101307:OnActionOver(caster, target, data)
...
-- 入场时
function Skill700101307:OnBorn(caster, target, data)
	if SkillJudger:CasterIsSelf(self, caster, target, true) then
	else
		return
	end
