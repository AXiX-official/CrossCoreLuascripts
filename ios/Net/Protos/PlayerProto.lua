...
function PlayerProto:UseItem(item, isOpenReward, func)
	...
	if item then
		local proto = {"PlayerProto:UseItem", {info = item}}
		NetMgr.net:Send(proto);
	else
...
function PlayerProto:UseItemRet(proto)
	if proto and proto.info and self.isOpenReward and proto.info.gets~=nil and #proto.info.gets>0 then
		UIUtil:OpenReward({proto.info.gets},{isNoShrink =not proto.isMerge});
	end
...
function PlayerProto:SaveTeam(teamData, callBack)
	...
	Log(sendData);
	local proto = {"PlayerProto:SetTeamData", {info = sendData}}
	NetMgr.net:Send(proto);
end
...
function PlayerProto:SaveTeamList(teamDatas, callBack)
	...
	local proto = {"PlayerProto:MultSetTeamData", {infos = sendDatas}}
	NetMgr.net:Send(proto);
end
...
function PlayerProto:BuyTeam()
	local proto = {"PlayerProto:BuyTeam", {count = 1}}
	NetMgr.net:Send(proto);
end
...
function PlayerProto:GetCardGetCnt(cfgId, callBack)
	...
	local proto = {"PlayerProto:GetCardGetCnt", {cfgid = cfgId}}
	NetMgr.net:Send(proto);
end
...
function PlayerProto:SetCardPoolSelCard(_card_pool_id, _cid)
	local proto = {"PlayerProto:SetCardPoolSelCard", {card_pool_id = _card_pool_id, cid = _cid}}
	NetMgr.net:Send(proto)
end
...
function PlayerProto:SectionMultiInfo(id)
	local proto = {"PlayerProto:SectionMultiInfo", {id = id}}
	NetMgr.net:Send(proto);
end
...
function PlayerProto:GetClientData(key)
	local proto = {"PlayerProto:GetClientData", {key = key}}
	NetMgr.net:Send(proto);
end
...
function PlayerProto:StartCombine(cfgId, num, callBack)
	...
	local proto = {"PlayerProto:StartCombine", {cfgid = cfgId, cnt = num}}
	NetMgr.net:Send(proto);
end
...
function PlayerProto:MainTalentUpgrade(_cid, _skill_id, _uf)
	local proto = {"PlayerProto:MainTalentUpgrade", {cid = _cid, skill_id = _skill_id, uf = _uf}}
	NetMgr.net:Send(proto)
end
...
function PlayerProto:RandSubTalent(_cid, _item_id)
	local proto = {"PlayerProto:RandSubTalent", {cid = _cid, item_id = _item_id}}
	NetMgr.net:Send(proto)
end
...
function PlayerProto:SetUseSubTalent(_cid, _types)
	local proto = {"PlayerProto:SetUseSubTalent", {cid = _cid, infos = _types}}
	NetMgr.net:Send(proto)
end

--替换副天赋(返回卡牌更新)
function PlayerProto:SetReplaceSubTalent(_cid, _is_replace, _type)
	local proto = {"PlayerProto:SetReplaceSubTalent", {cid = _cid, is_replace = _is_replace, type = _type}}
	NetMgr.net:Send(proto)
end
...
function PlayerProto:OpenSubTalentSlot(_cid, _index)
	local proto = {"PlayerProto:OpenSubTalentSlot", {cid = _cid, index = _index}}
	NetMgr.net:Send(proto)
end
...
function PlayerProto:PassCardRoleStory(_id, _index)
	local proto = {"PlayerProto:PassCardRoleStory", {id = _id, index = _index}}
	NetMgr.net:Send(proto)
end
...
function PlayerProto:SetCardTag(_cid, _tag)
	local proto = {"PlayerProto:SetCardTag", {cid = _cid, tag = _tag}}
	NetMgr.net:Send(proto)
end
...
function PlayerProto:UpgradeSubTalent(_cid, _index)
	local proto = {"PlayerProto:UpgradeSubTalent", {cid = _cid, index = _index}}
	NetMgr.net:Send(proto)
end
...
function PlayerProto:SetUseSubTalent(_cid, _indexs)
	local proto = {"PlayerProto:SetUseSubTalent", {cid = _cid, indexs = _indexs}}
	NetMgr.net:Send(proto)
end

--记录播放过的音效
function PlayerProto:AddRoleAudio(_id, _au_id)
	local proto = {"PlayerProto:AddRoleAudio", {id = _id, au_id = _au_id}}
	NetMgr.net:Send(proto)
end
...
function PlayerProto:GetAIStrategy(cids)
	local proto = {"PlayerProto:GetAIStrategy", {cid = cids}}
	NetMgr.net:Send(proto)
end
...
function PlayerProto:SetAIStrategy(list)
	local proto = {"PlayerProto:SetAIStrategy", {data = list}}
	NetMgr.net:Send(proto)
end
...
function PlayerProto:SwitchAIStrategy(nTeamIndex, nCardIndex, nStrategyIndex)
	local proto = {"PlayerProto:SwitchAIStrategy", {nTeamIndex = nTeamIndex,	
	nCardIndex = nCardIndex, nStrategyIndex = nStrategyIndex}}
	NetMgr.net:Send(proto)
end
...
function PlayerProto:GetCreateCardLogs(_card_pool_id,_skip, _GetCreateCardLogsCB)
	...
	local proto = {"PlayerProto:GetCreateCardLogs", {card_pool_id = _card_pool_id,skip =_skip}}
	NetMgr.net:Send(proto)
end
...
function PlayerProto:CardCoreLv(_cid, _uf, _CardCoreLvCB)
	local proto = {"PlayerProto:CardCoreLv", {cid = _cid, uf = _uf}}
	NetMgr.net:Send(proto)
end
...
function PlayerProto:SetCardInfo(_cid, _is_new, _cb)
	...
	local proto = {"PlayerProto:SetCardInfo", {cid = _cid, is_new = _is_new}}
	NetMgr.net:Send(proto)
end
...
function PlayerProto:LookRole(_role_id)
	local proto = {"PlayerProto:LookRole", {role_id = _role_id}}
	NetMgr.net:Send(proto)
...
function PlayerProto:GetBattleBossRank(page)
	local proto = {"PlayerProto:GetBattleBossRank",{nPage = page}}
	NetMgr.net:Send(proto)
end
