...
-- 回合开始处理完成后
function Skill4603504:OnAfterRoundBegin(caster, target, data)
	-- 8060
	if SkillJudger:CasterIsSelf(self, caster, target, true) then
	else
		return
...
function Skill4603504:tFunc_4603534_4603514(caster, target, data)
	...
	if self:Rand(4000+count737*100) then
		...
		for i,target in ipairs(targets) do
			self:CallOwnerSkill(SkillEffect[4603514], caster, target, data, 603500201+math.min(math.max(count738+count741-2,0),4))
		end
	end
end
function Skill4603504:tFunc_4603534_4603524(caster, target, data)
	...
	if self:Rand(4000+count737*100) then
		local targets = SkillFilter:Rand(self, caster, target, 4)
		for i,target in ipairs(targets) do
			self:CallOwnerSkill(SkillEffect[4603524], caster, target, data, 603500401+math.min(math.max(count738+count741-2,0),4))
		end
	end
end
