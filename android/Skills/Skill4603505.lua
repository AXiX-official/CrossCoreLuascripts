...
---------------------------------------------
-- 技能基类
Skill4603505 = oo.class(SkillBase)
function Skill4603505:Init(skillID, card)
	SkillBase.Init(self, skillID, card)
end
-- 回合开始时
function Skill4603505:OnRoundBegin(caster, target, data)
	-- 4603501
	self:AddBuffCount(SkillEffect[4603501], caster, self.card, data, 4603501,1,999)
end
-- 回合结束时
function Skill4603505:OnRoundOver(caster, target, data)
	-- 4603535
	self:tFunc_4603535_4603515(caster, target, data)
	self:tFunc_4603535_4603525(caster, target, data)
end
-- 攻击结束2
function Skill4603505:OnAttackOver2(caster, target, data)
	-- 8060
...
function Skill4603505:tFunc_4603535_4603525(caster, target, data)
	...
	-- 4603525
	if self:Rand(5000+count737*100) then
		local targets = SkillFilter:Rand(self, caster, target, 4)
		for i,target in ipairs(targets) do
			self:CallOwnerSkill(SkillEffect[4603525], caster, target, data, 603500401+math.min(math.max(count738+count741-2,0),4))
		end
	end
end
function Skill4603505:tFunc_4603535_4603515(caster, target, data)
	-- 8738
	local count738 = SkillApi:SkillLevel(self, caster, target,3,6035002)
	-- 8737
	local count737 = SkillApi:GetCount(self, caster, target,3,4603501)
	...
	if self:Rand(5000+count737*100) then
		local targets = SkillFilter:Rand(self, caster, target, 4)
		for i,target in ipairs(targets) do
			self:CallOwnerSkill(SkillEffect[4603515], caster, target, data, 603500201+math.min(math.max(count738+count741-2,0),4))
		end
	end
end
