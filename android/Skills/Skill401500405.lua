...
---------------------------------------------
-- 技能基类
Skill401500405 = oo.class(SkillBase)
function Skill401500405:Init(skillID, card)
	SkillBase.Init(self, skillID, card)
end
-- 执行技能
function Skill401500405:DoSkill(caster, target, data)
	-- 12001
	self.order = self.order + 1
	self:DamageLight(SkillEffect[12001], caster, target, data, 1,1)
	-- 401500405
	self.order = self.order + 1
	self:AddBuff(SkillEffect[401500405], caster, target, data, 401500405)
	-- 4401517
	local c618 = SkillApi:GetValue(self, caster, self.card,3,"c618")
	-- 4401515
	self.order = self.order + 1
	self:AddHp(SkillEffect[4401515], caster, target, data, math.floor(-c618))
	-- 8619
