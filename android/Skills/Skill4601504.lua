...
function Skill4601504:OnBefourHurt(caster, target, data)
	...
	-- 4601504
	self:AddTempAttr(SkillEffect[4601504], caster, caster, data, "damage",0.5)
end
-- 行动结束
...
-- 行动结束2
function Skill4601504:OnActionOver2(caster, target, data)
	-- 8071
	if SkillJudger:TargetIsFriend(self, caster, target, true) then
	else
		return
	...
	if self:Rand(7000) then
		self:BeatBack(SkillEffect[4601524], caster, self.card, data, nill,8)
	end
end
-- 攻击结束
function Skill4601504:OnAttackOver(caster, target, data)
	...
	-- 4601534
	self:AddBuffCount(SkillEffect[4601534], caster, self.card, data, 4601504,1,20)
end
