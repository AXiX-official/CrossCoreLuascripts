...
---------------------------------------------
-- 技能基类
Skill601900401 = oo.class(SkillBase)
function Skill601900401:Init(skillID, card)
	SkillBase.Init(self, skillID, card)
end
-- 执行技能
function Skill601900401:DoSkill(caster, target, data)
	-- 11004
	self.order = self.order + 1
	self:DamagePhysics(SkillEffect[11004], caster, target, data, 0.25,4)
end
-- 回合开始时
function Skill601900401:OnRoundBegin(caster, target, data)
	-- 601900303
...
-- 行动结束
function Skill601900401:OnActionOver(caster, target, data)
	-- 8060
	if SkillJudger:CasterIsSelf(self, caster, target, true) then
	else
		return
