...
function Skill4403901:OnAfterHurt(caster, target, data)
	...
	if self:Rand(4000) then
		self:AddBuffCount(SkillEffect[4403901], caster, self.card, data, 4403901,1,30)
	end
end
-- 回合开始时
function Skill4403901:OnRoundBegin(caster, target, data)
	-- 8062
	if SkillJudger:CasterIsTeammate(self, caster, target, true) then
	else
		return
	...
	if self:Rand(1000+math.max(math.floor((count7-180)/10)*100,0)) then
		self:AddBuff(SkillEffect[4403906], caster, caster, data, 9038)
	end
end
