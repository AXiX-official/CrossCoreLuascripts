...
---------------------------------------------
-- 技能基类
Skill403101302 = oo.class(SkillBase)
function Skill403101302:Init(skillID, card)
	SkillBase.Init(self, skillID, card)
end
-- 执行技能
function Skill403101302:DoSkill(caster, target, data)
	-- 12004
	self.order = self.order + 1
	self:DamageLight(SkillEffect[12004], caster, target, data, 0.25,4)
end
-- 行动开始
function Skill403101302:OnActionBegin(caster, target, data)
	-- 8060
...
-- 伤害后
function Skill403101302:OnAfterHurt(caster, target, data)
	-- 403100310
	self:tFunc_403100310_403100311(caster, target, data)
	self:tFunc_403100310_403100312(caster, target, data)
end
-- 伤害前
function Skill403101302:OnBefourHurt(caster, target, data)
	-- 8060
	if SkillJudger:CasterIsSelf(self, caster, target, true) then
	else
		return
	...
	-- 403101302
	self:AddTempAttr(SkillEffect[403101302], caster, self.card, data, "damage",count7*0.005)
	-- 8060
	if SkillJudger:CasterIsSelf(self, caster, target, true) then
	else
		return
...
function Skill403101302:tFunc_403100310_403100312(caster, target, data)
	-- 8060
	if SkillJudger:CasterIsSelf(self, caster, target, true) then
	else
		return
...
function Skill403101302:tFunc_403100310_403100311(caster, target, data)
	-- 8060
	if SkillJudger:CasterIsSelf(self, caster, target, true) then
	else
		return
