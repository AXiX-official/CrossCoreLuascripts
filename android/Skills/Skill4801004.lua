...
---------------------------------------------
-- 技能基类
Skill4801004 = oo.class(SkillBase)
function Skill4801004:Init(skillID, card)
	SkillBase.Init(self, skillID, card)
end
-- 伤害前
function Skill4801004:OnBefourHurt(caster, target, data)
	-- 8060
...
-- 行动结束
function Skill4801004:OnActionOver(caster, target, data)
	-- 8060
	if SkillJudger:CasterIsSelf(self, caster, target, true) then
	else
		return
...
-- 行动结束2
function Skill4801004:OnActionOver2(caster, target, data)
	-- 4603332
	self:AddStep(SkillEffect[4603332], caster, self.card, data, 1,1)
end
-- 特殊入场时(复活，召唤，合体)
function Skill4801004:OnBornSpecial(caster, target, data)
	-- 8060
	if SkillJudger:CasterIsSelf(self, caster, target, true) then
	else
		return
