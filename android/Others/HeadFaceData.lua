...
function this:CheckUse()
    local emotes = PlayerClient:GetEmotes()
...
-- 是否需要显示出来
function this:SelectToShow(selectType, selectData)
    if (selectType == 0 or selectType == 2) then
        return true
    elseif (selectType == 1) then
        local type = selectData + 1
        if (self:CheckCanUse() and (self:GetType() == 1 or type == self:GetType())) then
            return true
        end
    end
    return false
