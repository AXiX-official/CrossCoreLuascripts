...
function Awake()
    ...
    layout:Init("UIs/DungeonActivity11/DungeonCloudItem", LayoutCallBack, true)
...
function OnInit()
    UIUtil:AddTop2("DungeonCloud", topParent, OnClickBack,OnClickHome);
end
...
function CheckNew()
    if SectionNewUtil:IsNew("DungeonCloudView") then
        ...
        for k, v in ipairs(_datas) do
            ...
            if iconName and iconName~="" then
                ...
                elseif bgDatas[iconName] == nil then
                    ResUtil:CreateUIGOAsync("DungeonActivity11/" .. iconName,itemParent,function (go)
...
function ShowInfo(item)
    ...
    if itemInfo == nil then
        ResUtil:CreateUIGOAsync("DungeonActivity11/DungeonItemInfoCloud", infoParent, function(go)
...
function SetInfoItemPos()
    if itemInfo then
        itemInfo.SetPanelPos("CloudTitle",47,381)
        itemInfo.SetPanelPos("CloudLevel",47,314)
        itemInfo.SetPanelPos("CloudTarget",47,183)
    ...
    if currItem then
        ...
        if cfg then
            ...
            if cfgs and #cfgs > 1 then
                cfg = cfgs[itemInfo.CallFunc("CloudDanger","GetCurrDanger")]
            end
...
function ShowChangeLevel(cur,last)
    PlayAnim(800)
    if last and not IsNil(levelAnims[last]) then
        levelAnims[last]:SetBool("isSel",false)
    end
    if cur and not IsNil(levelAnims[cur]) then
