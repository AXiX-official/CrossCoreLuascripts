...
function RefreshPanel()
    SetItems()
    -- num
    local cur, max = curBuildData:GetNum_pet()
    CSAPI.SetText(txtNum, string.format("%s/%s", cur, max))
end

function SetItems()
    datas = curBuildData:GetRoleInfos_pet()
    items = items or {}
    ItemUtil.AddItems("Matrix/DormSetPetItem", items, datas, grid, OnItemClickCB, 1, curBuildData, function()
        ItemAnims()
    end)
end
...
function OnItemClickCB(_type, data)
    if (_type == 1) then
        -- 打开驻员列表
        CSAPI.OpenView("DormSetPetList", {curBuildData:GetID()})
    else
        -- 移除单个
        local ids = curBuildData:GetRoles_pet()
        local newIds = {}
